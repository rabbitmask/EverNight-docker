...
**数据库初始化文件**  
- `mysql/init/1-1-init.sql`（表结构 + 必需数据）  
...
🌤 20260111
修复前端地址复制失败的bug
修复任务定时执行每周/每月设置失败的bug
对端口扫描和DNS爆破扫描模块增加批量适配 重新设计批量前端展示
针对CT查询/FOFA查询三方接口的不稳定性加入限频器与自动重试机制

🌤 20260401
优化前端表格字段列宽无法手动调整的问题
优化评估任务-资产选择功能增加分页查询
优化资产列表和评估任务的资产选择功能增加全选按钮（全选筛选项）
优化首页卡片增加快捷跳转提高牛马效率
优化单任务超时上限，系统设计为异步并发，所以原超时并不影响已扫描结果，文案优化为“任务执行超时（已发现 37 条，已入池 12 条）”，单任务执行时间上限由2小时调整为6小时
前端硬编码收口，增加统一性设计
SOC联动，当前资产列表导出的表格可直接导入网站云监测/漏洞生命周期管理等soc系列产品
```